...
int main() {
    ...
    // particles within each computational block. Import the monodisperse test
    // packing and output the Voronoi tessellation in gnuplot and POV-Ray
    // formats.
    ...
    // Create a container for polydisperse particles using the same geometry as
    // above. Import the polydisperse test packing and output the Voronoi
    // radical tessellation in gnuplot and POV-Ray formats.
    container_poly_3d conp(x_min,x_max,y_min,y_max,z_min,z_max,n_x,n_y,n_z,
                           false,false,false,8);
