...
// "wall_3d" class. The "wall_3d" class contains virtual functions for cutting
// the Voronoi cell in response to a wall, and for telling whether a given
// point is inside the wall or not. In this derived class, specific
// implementations of these functions are given.
class wall_torus : public wall_3d {
    public:

        // The wall constructor initializes constants for the major and minor
        // axes of the torus. It also initializes the wall ID number that is
        // used when the plane cuts are made. This is only tracked with the
        // voronoicell_neighbor class and is ignored otherwise. It can be
        // omitted, and then an arbitrary value of -99 is used.
        wall_torus(double imjr,double imnr,int iw_id=-99)
            : w_id(iw_id), mjr(imjr), mnr(imnr) {};

        // This returns true if a given vector is inside the torus, and false
        // if it is outside. For the current example, this routine is not
        // needed, but in general it would be, for use with the point_inside()
        // routine in the container class.
        bool point_inside(double x,double y,double z) {
            double temp=sqrt(x*x+y*y)-mjr;
            return temp*temp+z*z<mnr*mnr;
        }

        // This template takes a reference to a voronoicell_3d or
        // voronoicell_neighbor_3d object for a particle at a vector
        ...
        inline bool cut_cell_base(vc_class &c,double x,double y,double z) {
            ...
            // Unless the particle is within 1% of the minor radius, then a
            // plane cut is made
            if(ot>1e-4*mnr*mnr) {
                ot=2*mnr/sqrt(ot)-2;
                z*=ot;
        ...
        // These virtual functions are called during the cell computation in
        // the container class. They call instances of the template given
        // above.
