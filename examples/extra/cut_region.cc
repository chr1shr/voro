...
int main() {
    ...
    // Now test over direction vectors from the center of the sphere. For each
    // vector, carry out a search to find the maximum distance along that
    // vector that a plane will intersect with cell, and save it to the output
    // file.
    for(theta=theta_step*0.5;theta<pi;theta+=theta_step) {
        ...
        for(phi=phi_step*0.5;phi<2*pi;phi+=phi_step) {
            ...
            // Now carry out a bisection search. Here, we initialize a minimum
            // and a maximum guess for the distance along this vector. Keep
            // multiplying rmax by two until the plane no longer makes a cut.
            rmin=0;rmax=1;
            ...
            // intersects, then move rmin to this point; otherwise, move rmax
            // there. At each stage the bounding interval is divided by two.
            // Exit when the width of the interval is smaller than the
            // tolerance.
