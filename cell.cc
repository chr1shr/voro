...
void voronoicell::add_memory(int i) {
	int s=2*i+1;
	if(mem[i]==0) {
		neighbor_allocate(i,init_n_vertices);
...
inline void voronoicell::check_relations() {
	int i,j;
	for(i=0;i<p;i++) {
		for(j=0;j<nu[i];j++) {
