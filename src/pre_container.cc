...
pre_container_base::pre_container_base(double ax_,double bx_,double ay_,double by_,double az_,double bz_,
    bool x_prd_,bool y_prd_,bool z_prd_,int ps_) :
    ax(ax_), bx(bx_), ay(ay_), by(by_), az(az_), bz(bz_),
    x_prd(x_prd_), y_prd(y_prd_), z_prd(z_prd_), ps(ps_),
...
/** Makes a guess at the optimal grid of blocks to use, by assuming that the
 * particles are evenly distributed in space, and aiming for the blocks to be
 * approximately cubes.
...
void pre_container::put(int n,double x,double y,double z) {
    if((x_prd||(x>=ax&&x<=bx))&&(y_prd||(y>=ay&&y<=by))&&(z_prd||(z>=az&&z<=bz))) {
