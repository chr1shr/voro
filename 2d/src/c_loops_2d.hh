...
class c_loop_base_2d {
    public:
        ...
        const int nxy;
        /** The number of floating point numbers per particle in the associated
         * container data structure. */
        const int ps;
        /** A pointer to the particle position information in the associated
         * container data structure. */
        double **p;
        /** A pointer to the particle ID information in the associated
         * container data structure. */
        int **id;
        /** A pointer to the particle counts in the associated container data
         * structure. */
        int *co;
         ...
         * block. */
        int q;
        /** The constructor copies several necessary constants from the base
         * container class.
         ...
         * \param[out] (x,y) the position vector of the particle.
         * \param[out] r the radius of the particle. If no radius information
        ...
        /** Returns the x position of the particle currently being considered
         * by the loop. */
        inline double x() {return p[ij][ps*q];}
        /** Returns the y position of the particle currently being considered
         * by the loop. */
        inline double y() {return p[ij][ps*q+1];}
        /** Returns the ID of the particle currently being considered by the
         * loop. */
...
class c_loop_all_2d : public c_loop_base_2d {
    ...
    private:
        /** Updates the internal variables to find the next computational block
         * with any particles.
         * \return True if another block is found, false if there are no more
         * blocks. */
...
class c_loop_subset_2d : public c_loop_base_2d {
    public:
        /** The current mode of operation, determining whether tests should be
         * applied to particles to ensure they are within a certain geometrical
         * object. */
...
class c_loop_order_2d : public c_loop_base_2d {
    public:
        ...
        int *op;
        /** The constructor copies several necessary constants from the base
         * class, and sets up a reference to the ordering class to use.
         * \param[in] con the container class to use.
    ...
    private:
        ...
        const int nx;
        /** Takes the current block index and computes indices in the x, y
         * directions. */
        inline void decode() {
            j=ij/nx;
            i=ij-j*nx;
